...
class PendaftaranController extends Controller
{
    ...
    public function store(PasienRequest $request)
    {
        ...
        try {
            ...
            return redirect(route('user.pendaftaran'))->with(['success' => "Pasien berhasil didaftarkan!"]);
        } catch (\Exception $e) {
            DB::rollback(); // Rollback transaksi jika terjadi kesalahan
            return redirect(route('user.pendaftaran'))->withErrors(['error' => "Terjadi kesalahan: " . $e->getMessage()]);
        }
    }
}
